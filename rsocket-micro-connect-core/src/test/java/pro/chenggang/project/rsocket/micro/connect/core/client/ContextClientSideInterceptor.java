 ...
 */
@Slf4j
public class ContextClientSideInterceptor implements RSocketExecutionBeforeInterceptor, RSocketExecutionAfterInterceptor {

    @Override
    public Mono<Void> interceptBefore(RSocketExchange exchange, RSocketInterceptorChain chain) {
        return Mono.deferContextual(contextView -> {
            log.info("[Context-Client-Side](Before):{},Context:{}", exchange, contextView);
    ...
    public Mono<Void> interceptAfter(RSocketExchange exchange, RSocketInterceptorChain chain) {
        return Mono.deferContextual(contextView -> {
            log.info("[Context-Client-Side](After):{},Context:{}", exchange, contextView);
            return chain.next(exchange);
        });
    }
}
